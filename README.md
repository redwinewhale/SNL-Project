...
AI 개발: 추론 엔진 모델링 및 프롬프트 엔지니어링

App UI 개발: 게임 화면 및 프론트엔드 디자인

App, Server, DB 개발 (본인): 클라이언트 로직(Phaser 3), REST API 설계, 데이터베이스 스키마 구축, Docker 샌드박스 연동
...
사양이 충분하여 로컬에서 Ollama(LLM) 서버와 Node.js 서버, DB를 동시에 구동하며 테스트가 가능합니다.
